    ...
    def draw(self, image: np.ndarray, course: Course) -> np.ndarray:
        ...
        for obj in course:
            ...
            if self.draw_masks:
                ...
                colored_mask[obj.mask > 0] = self.OBJECT_COLORS.get(obj.label, self.DEFAULT_COLOR) 
                cv2.addWeighted(colored_mask, self.mask_alpha, canvas, 1 - self.mask_alpha, 0, canvas)
