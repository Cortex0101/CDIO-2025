...
class AIModel:
    ...
    def generate_course(self, source):
        self.results = self.model.predict(source=source, verbose=False, conf=self.min_confidence, device=0, imgsz=640)
        return Course.from_yolo_results(self.results[0])
    
    def predict(self, source):
        return self.model.predict(source=source, verbose=False, conf=self.min_confidence, device=0, imgsz=640)